...
// up_proj:   [512, 2048] -> same
// down_proj: [2048, 512] -> same sizes
...
// 2-bit expert layout (not used for 35B)
#define EXPERT_SIZE_2BIT    EXPERT_SIZE  // placeholder — no 2-bit for 35B
#define GATE_W_OFF_2  GATE_W_OFF
#define GATE_S_OFF_2  GATE_S_OFF
#define GATE_B_OFF_2  GATE_B_OFF
#define UP_W_OFF_2    UP_W_OFF
#define UP_S_OFF_2    UP_S_OFF
#define UP_B_OFF_2    UP_B_OFF
#define DOWN_W_OFF_2  DOWN_W_OFF
#define DOWN_S_OFF_2  DOWN_S_OFF
#define DOWN_B_OFF_2  DOWN_B_OFF

// KV cache maximum context length
#define MAX_SEQ_LEN 1048576  // 1M context — only 10 full-attn layers need KV cache
...
static int g_skip_experts = 0;   // prefill optimization: skip expert I/O, shared expert only
...
typedef struct {
    ...
    #define NUM_FULL_ATTN_LAYERS (NUM_LAYERS / FULL_ATTN_INTERVAL)  // 10 for 35B
    ...
    #define NUM_LINEAR_LAYERS (NUM_LAYERS - NUM_FULL_ATTN_LAYERS)  // 30 for 35B
    id<MTLBuffer> buf_delta_state[NUM_LINEAR_LAYERS];   // [V_HEADS*KEY_DIM*VAL_DIM] float per layer
...
static void fused_layer_forward(
    ...
    // Prefill skip optimization: skip expert I/O entirely for intermediate prefill tokens.
    // Only the recurrent state (KV cache, delta-net) matters — hidden is overwritten by next embed.
    // Jump straight to shared-expert-only path (no SSD reads).
    if (g_skip_experts) {
        // Shared expert SwiGLU + down_proj (GPU or CPU — weights already in buf_shared_gate/up from CMD2)
        if (g_metal && sdw && sds && sdb) {
            ...
            gpu_encode_dequant_matvec_with_io_bufs(
                g_metal, cmd_shared, sdw, sds, sdb,
        ...
        // Combine: hidden = h_mid + shared_out * sigmoid(gate)
        float shared_weight = cpu_sigmoid(shared_gate_score);
        for (int i = 0; i < HIDDEN_DIM; i++)
            hidden[i] = h_mid[i] + shared_out[i] * shared_weight;
        if (g_timing_enabled) { t1 = now_ms(); g_timing.cmd3_encode += t1 - t0; g_timing.count++; g_timing.total += t1 - t_layer_start; }
        return;
    }
    ...
    uint16_t *final_norm_w, int K)
{
    ...
    for (;;) {
        ...
        if (strcmp(method, "POST") == 0 && strcmp(path, "/v1/chat/completions") == 0) {
            ...
            // Intermediate prefill tokens: run full forward but discard last-layer expert output
